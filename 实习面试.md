...
是做移动端适配用的方法，vw/vh就是视口的宽/高，vw把屏幕的宽等分成100份,如一个屏幕宽度为375,那么用vw编写网页就是, 3.75vw.
...
通常用来判断**基本数据类型**，它**返回表示数据类型的字符串**（返回结果只能包括number,boolean,string,function,undefined，object）； *注意，**使用typeof来判断null和引用类型的实例 返回的结果都是 'object'**
...
3. 检查宏任务队列，如果有任务，则重复以上步骤，否则等待新任务。
...
4. 其他⽅式：缓存、vuex、eventBus事件总线、provide / inject、父组件$refs获取子组件数据、子组件$parent获取父组件数据、    作用域插槽
