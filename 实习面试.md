...
![前端面试手册.assets/image-20210220214851409.png](前端面试手册.assets/image-20210220214851409.png)
...
Last-modify是一个时间标识，是该资源**最后修改的时间**，例如`Last-Modify: Thu,31 Dec 2037 23:59:59 GMT`。
...
ETag值的变更则说明资源状态已经被修改。clearFiles
...
3. 建立TCP连接后, 发起HTTP请求
4. 服务器响应HTTP请求，浏览器得到html代码bash
5. 浏览器解析html代码，并请求html代码中的资源（如js、css、图片等）（先得到7html代码，才能去找这些资源）
6. 浏览器对页面进行渲染呈现给用户
...
```js
    ...
    git merge [branchname]                合并分支（保留详细的合并信息）
    git rebase [branchname]               合并分支（更简洁的提交历史，但是代码问题不容易定位）
...
![](https://img-blog.csdnimg.cn/20210417225533423.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80Mjk5MTcxNg==,size_16,color_FFFFFF,t_70)

**vite原理图**：

![](https://img-blog.csdnimg.cn/20210417225611243.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80Mjk5MTcxNg==,size_16,color_FFFFFF,t_70)
...
vue数据双向绑定是通过数据劫持结合发布者-订阅者模式的⽅式来实现的.
 ...
 当数据改变了就会来触发这个函数；get方法用来获取值，set方法用来拦截设置值

发布订阅模式:
发布订阅者模式就是一种一对多的依赖关系。多个订阅者（一般是注册的函数）同时监听同一个数据对象，当这个数据对象发生变化的时候会执行一个发布事件，通过这个发布事件会通知到所有的订阅者，使它们能够自己改变对数据对象依赖的部分状态。
一个完整的订阅发布模式，由发布者、订阅者、消息管理器三部分组成，其中Vue实例或组件就是发布者。

![](images\Snipaste_2024-04-27_14-16-47.png)

## 说说vue中的diff算法？

diff算法 当data发生改变 会根据新的数据生成一个新的虚拟dom ，新的虚拟dom和旧的虚拟dom进行对比，这个对比的过程就是diff算法。

Vue2 是全量 Diff（当数据发生变化，它就会新生成一个DOM树，并和之前的DOM树进行比较，找到不同的节点然后更新。）；Vue3 是静态标记 + 非全量 Diff（Vue 3在创建虚拟DOM树的时候，会根据DOM中的内容会不会发生变化，比如常量，添加一个静态标记。静态节点只会在首次渲染时被处理，后续更新时会直接跳过比较和更新操作）
...
computed属性的属性值是⼀函数，函数返回值为属性的属性值，computed中每个属性都可以设置set与get⽅法。watch监听的数据必须是data中声明过或⽗组件传递过的数据
  ...
  watch: {  
    ...
    obj: {  
      handler(newVal, oldVal) {  
        console.log('obj 发生了变化:', newVal, oldVal);  
        // 执行其他操作...  
      },  
      deep: true, // 深度监听，监听 obj 内部属性的变化  
...
4. Proxy 返回的是一个新对象，我们可以只操作新的对象达到目的，而 Object.defineProperty 只能遍历 原对象 属性并直接修改达到目的
6. Object.defineProperty 兼容性好，支持 IE9等较旧的浏览器，而 Proxy 的存在浏览器兼容性问题, 而且无法用 polyfill 解决，因此，在一些需要兼容老版本浏览器的场景中，Object.defineProperty可能是一个更好的选择。
...
- 重写了 Vdom ，突破了 Vdom 的性能瓶颈。（这里指的是**diff算法**）
-
- 进行模板编译优化。
- 更加高效的组件初始化
...
methods: {  
  goToUser(userId) {  
    // 使用命名路由和params传递参数  
    this.$router.push({ name: 'user', params: { id: userId } });  // this.$route.params.userId
      

    // 或者使用path和query传递参数  
    // this.$router.push(`/user/${userId}`); // 这种方式也可以，但参数会出现在URL中  this.$route.params.userId
    // 使用query参数不会出现在URL的路径部分，而是作为查询字符串出现  
    // this.$router.push({ path: '/user', query: { id: userId } });  // this.$route.params.query
